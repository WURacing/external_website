  ...
  <TransitionRoot appear :show="isOpen" as="template">
    <Dialog as="div" @close="close" class="fixed inset-0 overflow-y-auto" :class="{ 'modal-open': isOpen }">
      ...
      <div class="relative inset-0 overflow-y-auto flex items-center justify-center p-4">
        <DialogPanel class="w-full max-w-5xl transform rounded-lg bg-stone-900 p-6 shadow-xl transition-all"
          ref="modalContainer">
          ...
          <div v-if="selectedItem">
            <p class="my-4 text-stone-100">{{ selectedItem.description }}</p>
            <p class="text-sm text-stone-100">{{ selectedItem.date }}</p>
          </div>
